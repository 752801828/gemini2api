...
- 账号管理：新增、删除、检测、Cookie 编辑、CK 更新时间、Flow 邮箱和来源。
- 模型测试/API Playground：直接生成完整请求示例并测试接口。
- Gem 管理、模型映射、API Key 管理、设置管理。
- 实时日志：状态、账号、模型、耗时、错误摘要和详情。
...
2026-08-11 基线：`238 passed, 1 xfailed`。已知 xfail 为生图意图短语边界误判，不影响 Flow 代理一致性。
