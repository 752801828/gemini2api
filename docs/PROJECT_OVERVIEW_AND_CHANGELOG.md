...
- Flow CK 获取：网络或 5xx 瞬时错误按 2–5 秒随机等待，最多 3 次；4xx 业务错误不重试，最终失败才告警。
- 空内容：按失败处理并重试。
...
- `upstream/v1.6.39`：合并上游协议兼容性、流式保活、扩展思考、工具调用重试、跨域 Cookie 冲突处理、完整请求/响应体日志开关及管理台设置持久化修复；保留现有 Flow Cookie bridge、固定代理路由、盘巡、账号同步、共享 `gemini-bridge` 网络、自定义重试策略及实时日志脱敏预览。验证：Python `compileall` 通过；完整测试 `649 passed, 1 skipped`，跳过项仅为 Windows 不支持 POSIX `chmod` 只读目录语义。

### 2026-08-27

- Flow CK 刷新遇到 `409` 等确定性 4xx 错误时立即停止，不再对同一账号连续请求 3 次；网络和 5xx 临时故障仍保留最多 3 次重试。减少 Flow2API 数据库连接与线程压力，部署需重建 Gemini2API 容器。

### 2026-08-17

- 修复升级到 `1.6.32` 后浏览器仍复用旧“模型测试”HTML、看不到“思考”开关的问题：同步提升组件加载器脚本和所有动态组件的缓存版本；不改变模型调用逻辑。部署后需要更新静态文件并刷新管理页面；新增缓存键与开关存在性回归测试。
