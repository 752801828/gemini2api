...
@pytest.mark.skipif(os.name == "nt", reason="Windows chmod does not enforce POSIX directory write bits")
def test_readonly_data_dir_leaves_env_and_memory_untouched(client, disk_layers):
    """data/ 不可写 → 500，.env 与内存都回到请求前。"""
    object.__setattr__(settings, "max_retries", 1)
    disk_layers.env.write_text("MAX_RETRIES=1\n", encoding="utf-8")
    env_before = disk_layers.env.read_bytes()
